...
export default function ConceptMap({ extractedText, keywords, fileName }: ConceptMapProps) {
  ...
  const generateConceptMap = async () => {
    ...
    setError('')
    setProgress(0)
    setStatusMessage('Initializing concept map generation...')
    setHasGenerated(true)
    
    try {
      // Simulate progress updates (slower and more gradual)
      const progressSteps = [
        { progress: 15, message: 'Analyzing document and extracting concepts...' },
        { progress: 30, message: 'Processing keywords and relationships...' },
        { progress: 45, message: 'Generating concept nodes...' },
        { progress: 60, message: 'Calculating node positions...' },
        { progress: 75, message: 'Creating connections between concepts...' },
        { progress: 90, message: 'Finalizing concept map layout...' },
        { progress: 95, message: 'Preparing visualization...' }
      ]
      ...
      }, 1200) // Slower interval (was 600ms)
      ...
      setProgress(100)
      setStatusMessage('Concept map generated successfully!')
    ...
    return (
      ...
      >
        ...
        {links.map((link, index) => {
          ...
          const sourceRadius = getNodeSize(sourceNode.importance) / 2
          const targetRadius = getNodeSize(targetNode.importance) / 2
          ...
          // Calculate line endpoints (start from source node edge, end at target node edge)
          ...
          return (
            <g key={`link-${index}`}>
              <line
                ...
                y2={endY}
                stroke="#6B7280"
                strokeWidth={Math.max(2, link.strength * 4)}
                opacity="0.8"
  ...
  // Remove automatic generation - user must click generate button
  // useEffect(() => {
        ...
        <div ref={containerRef} className="flex-1 flex items-center justify-center">
          {isLoading ? (
            <div className="text-center w-full max-w-md">
              ...
              {/* Status Message */}
              <p className="text-gray-600 dark:text-gray-400 mb-2">{statusMessage}</p>
            ...
            <div className="text-center max-w-2xl">
              <Brain className="w-20 h-20 mx-auto mb-6 text-purple-500 dark:text-purple-400" />
              <h3 className="text-2xl font-semibold text-gray-800 dark:text-gray-200 mb-4">
                Concept Map Generator
              </h3>
              <p className="text-gray-600 dark:text-gray-400 mb-6 leading-relaxed">
                Transform your document into an interactive visual network showing how key concepts, methods, and keywords connect and relate to each other.
              </p>
              <button
                onClick={generateConceptMap}
                disabled={!extractedText || keywords.length === 0}
                className="px-6 py-3 bg-purple-500 text-white rounded-lg hover:bg-purple-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center gap-2 mx-auto"
              >
                <Brain className="w-5 h-5" />
                Generate Concept Map
              </button>
              {(!extractedText || keywords.length === 0) && (
                <p className="text-sm text-gray-500 dark:text-gray-500 mt-3">
                  Upload a PDF and extract keywords first
                </p>
              )}
            </div>
          ) : conceptMapData.nodes.length === 0 ? (
            <div className="text-center">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-500 mb-4">
                Click the generate button to create your concept map
              </p>
              <button
                onClick={generateConceptMap}
